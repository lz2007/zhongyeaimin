...
Page({
  ...
  load(storeindex,goodindex){
    ...
    app.request({ url: 'store/all' }).then(res => {
      self.setData({
        store: res
      })
      // console.log(storeindex) 
      app.request({ url: 'store/catgory/' + self.data.store[storeindex].id }).then(res => {
        ...
        if (res.catgory.length !== 0) {
          ...
          res.catgory.map((item, index) => {
            // + '/?catgory=' + item.id
            app.request({ url: 'store/' + self.data.store[storeindex].id  }).then(res => {
              console.log(res)
      ...
      app.request({ url: 'store/' + res[0].id }).then(res => {
        console.log(res)
        this.setData({
          ['Coupon[0]']:res.data
        })
